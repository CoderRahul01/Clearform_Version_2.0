...
export const toggleTrackClassName = (on) => (on ? 'bg-[#2a9d6e]' : 'bg-[#e4e2dc]');

const ToggleSwitch = ({ checked, onChange, className = '', size = 'md' }) => {
  const dims =
    size === 'sm'
  ...
  return (
    <button
      ...
      className={`relative inline-flex shrink-0 items-center rounded-full transition-colors cursor-pointer focus:outline-none appearance-none border-0 p-0 ${dims.track} ${toggleTrackClassName(checked)} ${className}`}
    >
      <span
